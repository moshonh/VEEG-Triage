...
# ── טעינת API Key מ-Secrets (מוסתר מהמשתמש) ─────────────────────────────────
try:
    api_key = st.secrets["GROQ_API_KEY"]
except (KeyError, FileNotFoundError):
    api_key = None

# ── Sidebar (ללא שדה API Key – מוסתר) ────────────────────────────────────────
with st.sidebar:
    st.header("⚙️ הגדרות מערכת")

    if not api_key:
        st.error("⚠️ API Key לא מוגדר. פנה למנהל המערכת.", icon="🔑")
...
with col_warn:
    if not api_key:
        st.warning("⬅️ API Key לא מוגדר – פנה למנהל המערכת", icon="⚠️")
...
if submit_btn:
    ...
    if not seizure_frequency:
        errors.append("יש לבחור תדירות התקפים.")
    if not api_key:
